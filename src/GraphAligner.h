...
public:
	...
	AlignmentResult AlignClusters(const std::string& seq_id, const std::string& sequence, const std::vector<SeedCluster>& seedClusters, AlignerGraphsizedState& reusableState) const
	{
		...
		for (size_t i = 0; i < seedClusters.size(); i++)
		{
			...
			if (seedClusters[i].size() < params.minSeedClusterSize)
			{
	...
	{
		assert(alignment.trace->trace.size() > 0);
		alignment.GAFline = GAFAlignment::traceToAlignment(seq_id, sequence, *alignment.trace, alignment.alignmentXScore, alignment.mappingQuality, params, cigarMatchMismatchMerge);
	}
	...
	std::vector<SeedCluster> clusterSeeds(const std::vector<SeedHit>& seedHits) const
	{
