...
private:
	...
	OnewayTrace getReverseTraceFromTableExactEndPos(const std::string_view& sequence, const DPTable& slice, AlignerGraphsizedState& reusableState) const
	{
		...
		EqVector EqV = BV::getEqVector(sequence, slice.slices[bestIndex].j);
		std::vector<WordSlice> nodeSlices = recalcNodeWordslice(node, slice.slices[bestIndex].scores.node(node), EqV, previous);
	...
	OnewayTrace getReverseTraceFromTable(const std::string_view& sequence, const DPTable& slice, AlignerGraphsizedState& reusableState, MatrixPosition startPos, ScoreType startScore) const
	{
		...
		EqVector EqV = BV::getEqVector(sequence, 0);
		while (result.trace.back().DPposition.seqPos != (size_t)-1)
		{
			...
			{
				if (newSlice != currentSlice) EqV = BV::getEqVector(sequence, slice.slices[newSlice].j);
				...
				nodeSlices = recalcNodeWordslice(currentNode, slice.slices[currentSlice].scores.node(currentNode), EqV, previous);
#ifdef SLICEVERBOSE
			...
			{
				auto bt = pickBacktraceCorner(slice.slices[currentSlice].scores, slice.slices[currentSlice-1].scores, currentNode, slice.slices[currentSlice].j, sequence, slice.slices[currentSlice].minScore + slice.slices[currentSlice].bandwidth, slice.slices[currentSlice].scoresNotValid, slice.slices[currentSlice-1].minScore + slice.slices[currentSlice-1].bandwidth, slice.slices[currentSlice-1].scoresNotValid);
			...
			assert(result.trace.back().DPposition.nodeOffset != 0);
			assert(result.trace.back().DPposition.seqPos % WordConfiguration<Word>::WordSize != 0);
			auto inner = pickBacktraceInside(slice.slices[currentSlice].j, nodeSlices, result.trace.back().DPposition, sequence);
			for (auto pos : inner)
			{
	...
	std::pair<std::pair<MatrixPosition, bool>, std::pair<MatrixPosition, bool>> pickBacktraceHorizontalCrossing(const NodeSlice<LengthType, ScoreType, Word, false>& current, const NodeSlice<LengthType, ScoreType, Word, false>& previous, size_t j, LengthType node, MatrixPosition pos, const std::string_view& sequence, ScoreType quitScore, bool scoresNotValid, ScoreType previousQuitScore, bool previousScoresNotValid) const
	{
		...
		if (offset == 0)
		{
			return std::make_pair(std::make_pair(pos, false), pickBacktraceCorner(current, previous, node, j, sequence, quitScore, scoresNotValid, previousQuitScore, previousScoresNotValid));
		}
	...
	std::vector<WordSlice> recalcNodeWordslice(LengthType node, const typename NodeSlice<LengthType, ScoreType, Word, false>::NodeSliceMapItem& slice, const EqVector& EqV, const typename NodeSlice<LengthType, ScoreType, Word, false>::NodeSliceMapItem& previousSlice) const
	{
		size_t nodeLength = params.graph.NodeLength(node);
		std::vector<bool> previousBand; // fake!
		...
		{
			calculateNodeInner<false, false>(node, sliceCopy, EqV, previousSlice, incoming, previousBand, params.graph.NodeChunks(node), [&result](const WordSlice& slice) { result.push_back(slice); });
		}
		else
		{
			calculateNodeInner<false, false>(node, sliceCopy, EqV, previousSlice, incoming, previousBand, params.graph.AmbiguousNodeChunks(node), [&result](const WordSlice& slice) { result.push_back(slice); });
		}
	...
	{
		return calculateNodeInner<PreciseClipping, true>(i, slice, EqV, previousSlice, incoming, previousBand, nodeChunks, [](const WordSlice& slice){});
	}

	template <bool PreciseClipping, bool AllowEarlyLeave, typename NodeChunkType, typename WordsliceCallback>
#ifdef NDEBUG
	__attribute__((always_inline))
#endif
	NodeCalculationResult calculateNodeInner(size_t i, typename NodeSlice<LengthType, ScoreType, Word, true>::NodeSliceMapItem& slice, const EqVector& EqV, typename NodeSlice<LengthType, ScoreType, Word, true>::NodeSliceMapItem previousSlice, const std::vector<EdgeWithPriority>& incoming, const std::vector<bool>& previousBand, NodeChunkType nodeChunks, WordsliceCallback callback) const
	{
	...
	NodeCalculationResult calculateSlice(const std::string_view& sequence, size_t j, NodeSlice<LengthType, ScoreType, Word, HasVectorMap>& currentSlice, const NodeSlice<LengthType, ScoreType, Word, PreviousHasVectorMap>& previousSlice, std::vector<bool>& currentBand, const std::vector<bool>& previousBand, PriorityQueue& calculableQueue, ScoreType previousQuitScore, int bandwidth, ScoreType previousMinScore) const
	{
		...
		while (calculableQueue.size() > 0)
		{
			...
			if (i < params.graph.firstAmbiguous)
			{
				...
				{
					nodeCalc = calculateNode<false>(i, thisNode, EqV, previousThisNode, *extras, previousBand, params.graph.NodeChunks(i));
				}
			}
			else
			{
				...
				{
					nodeCalc = calculateNode<false>(i, thisNode, EqV, previousThisNode, *extras, previousBand, params.graph.AmbiguousNodeChunks(i));
				}
			}
			calculableQueue.pop();
			if (!calculableQueue.IsComponentPriorityQueue())
	...
	void flattenLastSliceEnd(NodeSlice<LengthType, ScoreType, Word, HasVectorMap>& slice, const NodeSlice<LengthType, ScoreType, Word, PreviousHasVectorMap>& previousSlice, NodeCalculationResult& sliceCalc, LengthType j, const std::string_view& sequence) const
	{
		...
		for (auto node : slice)
		{
			...
			std::vector<WordSlice> nodeSlices = recalcNodeWordslice(node.first, current, EqV, old);
