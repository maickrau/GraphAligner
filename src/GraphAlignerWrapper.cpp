...
AlignmentResult AlignOneWay(const AlignmentGraph& graph, const std::string& seq_id, const std::string& sequence, size_t initialBandwidth, bool quietMode, GraphAlignerCommon<size_t, int32_t, uint64_t>::AlignerGraphsizedState& reusableState, double preciseClippingIdentityCutoff, int Xdropcutoff, size_t DPRestartStride)
{
	GraphAlignerCommon<size_t, int32_t, uint64_t>::Params params {initialBandwidth, graph, std::numeric_limits<size_t>::max(), quietMode, false, 1, 0, preciseClippingIdentityCutoff, Xdropcutoff, 0};
...
{
	GraphAlignerCommon<size_t, int32_t, uint64_t>::Params params {initialBandwidth, graph, maxCellsPerSlice, quietMode, sloppyOptimizations, minClusterSize, seedExtendDensity, preciseClippingIdentityCutoff, Xdropcutoff, multimapScoreFraction};
	GraphAligner<size_t, int32_t, uint64_t> aligner {params};
...
{
	GraphAlignerCommon<size_t, int32_t, uint64_t>::Params params {initialBandwidth, graph, maxCellsPerSlice, quietMode, sloppyOptimizations, minClusterSize, seedExtendDensity, preciseClippingIdentityCutoff, Xdropcutoff, 0};
	GraphAligner<size_t, int32_t, uint64_t> aligner {params};
