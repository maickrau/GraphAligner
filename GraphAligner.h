...
#ifndef NDEBUG
		if (std::get<1>(trace.first).size() > 0) verifyTrace(std::get<1>(trace.first), sequence, std::get<0>(trace.first));
		if (std::get<1>(trace.second).size() > 0) verifyTrace(std::get<1>(trace.second), sequence, std::get<0>(trace.second));
#endif
